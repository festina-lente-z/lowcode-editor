<template>
  ...
  >
    ...
    <template v-else>
      ...
      <div 
        class="min-h-[100px]"
        :style="{ flex: `0 0 ${config.centerColSpan}fr` }"
      >
        <draggable
          v-model="centerBlockList"
          item-key="id"
          class="space-y-3"
          ghost-class="draggable-ghost"
          group="blocks"
          animation="200"
          handle=".drag-handle"
          @end="handleCenterDragEnd"
          @change="handleCenterDragChange"
        >
          <template #item="{ element }">
            <BlockItem
              :block="element"
...
const centerBlockList = ref<BlockSchema[]>([])
const rightBlockList = ref<BlockSchema[]>([])

watch(() => props.blocks, (newBlocks) => {
  const third = Math.floor(newBlocks.length / 3)
  leftBlockList.value = newBlocks.slice(0, third)
  centerBlockList.value = newBlocks.slice(third, third * 2)
  rightBlockList.value = newBlocks.slice(third * 2)
...
const handleLeftDragEnd = () => {
  const allBlocks = [...leftBlockList.value, ...centerBlockList.value, ...rightBlockList.value]
...
const handleLeftDragChange = (event: any) => {
  if (event.added) {
    const allBlocks = [...leftBlockList.value, ...centerBlockList.value, ...rightBlockList.value]
    const reordered = allBlocks.map.map((block, index) => ({ ...block, order: index }))
    emit('reorder', reordered)
  }
}
...
const handleCenterDragChange = (event: any) => {
  if (event.added) {
    const allBlocks = [...leftBlockList.value, ...centerBlockList.value, ...rightBlockList.value]
    const reordered = allBlocks.map((block, index) => ({ ...block, order: index }))
    emit('reorder', reordered)
